...
# Define CORS middleware to allow cross-domain access to API
import json
...
def start():
    ...
    try:
        ...
        os.system("uvicorn vicky_app:app --host 127.0.0.1 --port 8000 --workers 4")
    except Exception as e:
        logger.error(f"Failed to start server: {e}")
        print(f"\nError starting the server: {e}")
        print("\nTroubleshooting tips:")
